    ...
    <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Deep dive into platform growth and system performance
            </p>
          </div>
          <button
            onClick={fetchData}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-card border border-border rounded-2xl p-6 relative overflow-hidden group">
          ...
          <p className="text-3xl font-bold text-foreground">
            {data?.rfpDistribution?.reduce(
              (a: any, b: any) => a + b._count.id,
              0,
            ) || 0}
          </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-card border border-border rounded-2xl p-6 shadow-sm">
          <h3 className="text-sm font-bold mb-6 flex items-center gap-2">
            <Database className="w-4 h-4 text-emerald-500" /> RFP Distribution
            by Category
          </h3>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  ...
                  nameKey="category"
                >
                  {data?.rfpDistribution?.map((_entry: any, index: number) => (
                    <Cell
                      key={`cell-${index}`}
        ...
        <div className="bg-card border border-border rounded-2xl p-6 shadow-sm">
          <h3 className="text-sm font-bold mb-6 flex items-center gap-2">
            <TrendingUp className="w-4 h-4 text-amber-500" /> Subscription
            Revenue (Monthly)
          </h3>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="bg-card border border-border rounded-2xl p-6 shadow-sm">
          <h3 className="text-sm font-bold mb-6 flex items-center gap-2">
            <Cpu className="w-4 h-4 text-purple-500" /> Infrastructure Resource
            Usage
          </h3>
            ...
            <div>
              ...
              <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min((health?.memoryUsage?.rss / 512 / 1024 / 1024) * 100, 100)}%`,
                  }}
                ></div>
              </div>
            </div>
            <div>
              ...
              <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min((health?.memoryUsage?.heapUsed / 256 / 1024 / 1024) * 100, 100)}%`,
                  }}
                ></div>
              </div>
            </div>
      ...
      <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
        <div className="p-6 border-b border-border flex justify-between items-center">
          <h3 className="text-sm font-bold flex items-center gap-2">
            <Activity className="w-4 h-4 text-primary" /> Recent Real-time
            Activity
          </h3>
          <span className="text-[10px] bg-primary/10 text-primary px-2 py-1 rounded-lg font-mono">
            LIVE_STREAM
          </span>
        </div>
            ...
            <tbody className="text-xs divide-y divide-border">
              {data?.recentActivity?.map((log: any) => (
                ...
                >
                  ...
                  <td className="px-6 py-3 font-mono text-primary/80">
                    {log.action}
                  </td>
