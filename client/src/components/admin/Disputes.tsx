...
const Disputes: React.FC = () => {
  ...
  const [resolvingStatus, setResolvingStatus] = useState<"RESOLVED" | "CANCELLED">("RESOLVED");
  ...
  const fetchDisputes = async () => {
    setLoading(true);
    try {
      const res = await getDisputes(filterStatus === "ALL" ? undefined : filterStatus);
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h2 className="text-2xl font-bold flex items-center gap-2">
              <ShieldAlert className="w-6 h-6 text-primary" /> Dispute Resolution Portal
            </h2>
            <p className="text-muted-foreground text-sm">Mediate and resolve reports between users</p>
          </div>
          <button 
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="lg:col-span-1 w-full lg:w-1/3 space-y-4">
          ...
          <div className="space-y-3 max-h-[600px] overflow-y-auto pr-2">
            {loading ? (
              <div className="p-10 text-center animate-pulse text-muted-foreground">Loading Disputes...</div>
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${getStatusColor(dispute.status)}`}>
                    {dispute.status}
                  </span>
                  <span className="text-[10px] text-muted-foreground">{new Date(dispute.createdAt).toLocaleDateString()}</span>
                </div>
                <h4 className="font-bold text-sm truncate">{dispute.reason}</h4>
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <span className="text-xs text-muted-foreground truncate">{dispute.reporter.firstName} vs {dispute.reported.firstName}</span>
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="lg:col-span-2 flex-1">
          {selectedDispute ? (
            <div className="bg-card border border-border rounded-xl overflow-hidden animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="p-6 border-b border-border bg-muted/30">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="text-xl font-bold">{selectedDispute.reason}</h3>
                    <p className="text-xs text-muted-foreground mt-1">Dispute ID: {selectedDispute.id}</p>
                  </div>
                  <span className={`px-3 py-1 rounded-full text-xs font-bold border ${getStatusColor(selectedDispute.status)}`}>
              ...
              <div className="p-6 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="p-4 bg-muted/20 rounded-xl border border-border">
                    ...
                    <p className="font-bold">{selectedDispute.reporter.firstName} {selectedDispute.reporter.lastName}</p>
                  ...
                  <div className="p-4 bg-muted/20 rounded-xl border border-border">
                    ...
                    <p className="font-bold">{selectedDispute.reported.firstName} {selectedDispute.reported.lastName}</p>
                    <p className="text-xs text-muted-foreground">{selectedDispute.reported.email}</p>
                  </div>
                </div>

                <div className="space-y-2">
                  <h4 className="text-sm font-bold flex items-center gap-2">
                    <MessageSquare className="w-4 h-4 text-primary" /> Dispute Description
                  </h4>
                  ...
                  <div className="p-4 bg-primary/5 border border-primary/10 rounded-xl">
                    ...
                    {selectedDispute.rfp && (
                      <div className="flex items-center justify-between">
                        <span className="text-sm">RFP: <span className="font-bold">{selectedDispute.rfp.title}</span></span>
                        <button className="text-[10px] font-bold text-primary hover:underline">VIEW RFP</button>
                      </div>
                    )}
                  </div>
                )}
                ...
                {selectedDispute.status !== "RESOLVED" && selectedDispute.status !== "CANCELLED" ? (
                  <div className="pt-6 border-t border-border space-y-4">
                    <h4 className="font-bold flex items-center gap-2">
                      <Gavel className="w-5 h-5 text-primary" /> Admin Resolution
                    </h4>
                    ...
                    >
                      {isSubmitting ? 'Processing...' : `Submit ${resolvingStatus === 'RESOLVED' ? 'Resolution' : 'Dismissal'}`}
                    </button>
                  </div>
                ) : (
                  <div className="pt-6 border-t border-border space-y-4">
                    <div className="p-6 bg-green-500/5 border border-green-500/20 rounded-2xl">
                      ...
                      <p className="text-sm text-green-700/80 leading-relaxed">{selectedDispute.resolution}</p>
                      <div className="mt-4 pt-4 border-t border-green-500/10 flex items-center justify-between text-[10px] text-green-600/60 font-mono">
                        <span>RESOLVED_BY: {selectedDispute.resolvedBy?.firstName} {selectedDispute.resolvedBy?.lastName}</span>
                        <span>DATE: {new Date(selectedDispute.updatedAt || selectedDispute.createdAt).toLocaleDateString()}</span>
                      </div>
                    </div>
                  </div>
                )}
            ...
            <div className="h-full border-2 border-dashed border-border rounded-xl flex flex-col items-center justify-center p-12 text-center text-muted-foreground bg-muted/5">
              ...
              <p className="text-sm max-w-xs mt-2">Select a dispute from the list to review evidence and issue a resolution.</p>
            </div>
          )}
        </div>
      </div>
